    ...
    public changeStatus(): void {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.time < (songProgress + offset)) {
                ...
                const request = Settings.view.advanced.enabled
                ...
                // Only mark as sent after Discord confirms — prevents permanent lock if request fails
                request.then((res) => {
                    if (res.ok) {
                        this.sentLines.push(line)
                    } else {
                        // Request failed (401, 429, etc) — reset so next tick retries
