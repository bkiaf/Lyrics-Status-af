const THEMES = {
  dark: {
    ...
    vars: {
      ...
      '--accent': '#9298ff', '--accent-vivid': '#7f86ff',
      '--accent-bg': 'rgba(146,152,255,0.07)', '--accent-border': 'rgba(146,152,255,0.2)',
    }
  },
  purple: {
    label: 'Royal Violet',
...
<div id="app">
  <div class="sidebar">
    <div class="brand">
      ...
      <span id="version-badge" class="version-badge">7.1.2</span>
    </div>
    <nav class="nav">
  ...
  <div class="content">
    <div class="tab active" id="tab-auth">
      ...
      <div class="card anim-card" style="margin-top:12px;">
        ...
        <div class="card-section">
          ...
          <input type="text" id="custom-redirect-uri" class="input" placeholder="http://127.0.0.1:8999/callback">
        ...
        <div class="card-section">
          <div class="toggle-group">
            ...
            <label class="toggle-row">
              ...
              <span class="toggle-track"><span class="toggle-thumb"></span></span>
              <div>
                <span class="toggle-label">Show lyrics prefix</span>
                <span class="field-hint">Adds <code>Song lyrics -</code> before the lyric line, not the song title.</span>
              </div>
            </label>
          </div>
        </div>
    ...
    <div class="tab" id="tab-lyrics">
      <div class="tab-header">
        <h1>Lyrics</h1>
        <p>Preview the current synced lyrics, then fix any wrong words and save them for this exact Spotify track.</p>
      </div>

      <div class="card anim-card lyrics-card lyrics-panel-card" id="lyrics-card">
        <div class="lyrics-card-head">
          <div>
            <div class="card-label">Live lyrics</div>
            <div class="lyrics-song-meta" id="lyrics-song-meta">Play a song to preview lyrics here.</div>
          </div>
          <div class="lyrics-head-actions">
            <div class="lyrics-animation-control" role="group" aria-label="Lyrics animation mode">
              <span>Animation</span>
              <input type="hidden" id="lyrics-animation-mode" value="leap">
              <div class="lyrics-animation-switch">
                <button type="button" class="lyrics-animation-option active" data-lyrics-animation="leap">Leap in</button>
                <button type="button" class="lyrics-animation-option" data-lyrics-animation="glow">Glow wave</button>
              </div>
            </div>
            <button id="lyrics-edit-btn" class="icon-btn lyrics-edit-btn" title="Edit lyrics" aria-label="Edit lyrics">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
            </button>
          </div>
        </div>
        <div class="divider"></div>
        <div class="card-section lyrics-card-body">
          <div id="lyrics-viewer" class="lyrics-viewer">
            <div class="lyrics-empty">Waiting for synced lyrics…</div>
          </div>
          <div id="lyrics-editor" class="lyrics-editor hid">
            <div class="lyrics-editor-topbar">
              <div>
                <div class="lyrics-editor-title">Edit synced lyrics</div>
                <p class="field-hint">Edit the words, then tune how long each word stays so the highlight matches the song.</p>
              </div>
              <button id="lyrics-cancel-btn" class="btn btn-secondary">Cancel</button>
            </div>
            <div id="lyrics-editor-list" class="lyrics-editor-list"></div>
            <textarea id="lyrics-editor-text" class="input textarea lyrics-editor-text hid" spellcheck="false" placeholder="[00:12.34] First lyric line"></textarea>
            <div class="lyrics-editor-actions">
              <button id="lyrics-save-btn" class="btn btn-primary">
                <span class="btn-label">Save all lyrics</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <div class="tab" id="tab-timing">
      ...
      <div class="card anim-card">
        ...
        <div class="divider"></div>
        <div class="card-section">
          <div class="section-row timing-offset-grid">
            <div class="timing-field">
              ...
              <input type="number" id="send-time-offset" class="input timing-number-input" value="500">
            </div>
            <div class="timing-field">
              <label class="field-label">Auto-offset samples</label>
              <p class="field-hint">Number of recent requests to average over.</p>
              <input type="number" id="autooffset" class="input timing-number-input" min="1" max="20">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card anim-card">
        ...
        <div class="card-section">
          <div class="preset-grid">
            <button class="theme-preset active" data-key="dark" aria-label="Use Midnight Violet theme">
              <span class="preset-preview" style="--s1:#0d0d0f;--s2:#141417;--s3:#9298ff;--s4:#7f86ff;--a3:rgba(146,152,255,0.42);--a4:rgba(127,134,255,0.30);">
                <span class="preset-orb"></span>
                <span class="preset-lines"><i></i><i></i><i></i></span>
              </span>
              <span class="preset-info">
                <span class="preset-label">Midnight Violet</span>
                <span class="preset-sub">clean dark + soft purple</span>
              </span>
          ...
          <div class="color-row">
            <div class="color-field">
              ...
              <div class="color-input-wrap">
                <input type="color" id="cp-accent" class="color-picker" value="#9298ff">
                <span class="color-hex" id="hex-accent">#9298ff</span>
              </div>
...
let userTokenInput          = $("#user-token"),
    ...
    lyricsViewer            = $("#lyrics-viewer"),
    lyricsSongMeta          = $("#lyrics-song-meta"),
    lyricsEditBtn           = $("#lyrics-edit-btn"),
    lyricsEditor            = $("#lyrics-editor"),
    lyricsEditorList        = $("#lyrics-editor-list"),
    lyricsEditorText        = $("#lyrics-editor-text"),
    lyricsSaveBtn           = $("#lyrics-save-btn"),
    lyricsCancelBtn         = $("#lyrics-cancel-btn"),
    lyricsAnimationMode     = $("#lyrics-animation-mode"),
    lyricsAnimationOptions  = $("[data-lyrics-animation]");

let settings = {
    credentials: {
        token: "", cookies: "", clientID: "", clientSecret: "",
        useExternalAuthServer: false, code: "", refreshToken: "", uuid: "", customRedirectUri: "http://127.0.0.1:8999/callback"
    },
    view: {
        timestamp: true, label: true, lyricsAnimation: "leap",
...
const TAB_ORDER = ["auth", "display", "lyrics", "timing", "theme", "update"];
...
authorizeButton.click(() => {
    if (settings.credentials.useExternalAuthServer) {
        ...
        startSpotifyAuthWatcher();
    } else {
        const clientId = settings.credentials.clientID;
        const redirectUri = settings.credentials.customRedirectUri || "http://127.0.0.1:8999/callback";
        if (!clientId) {
            setUpdStatus("Add your Spotify Client ID first.", "error", false);
            return;
        }
        window.open(`https://accounts.spotify.com/authorize?client_id=${encodeURIComponent(clientId)}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent("user-read-playback-state user-read-currently-playing")}`, '_blank');
        startSpotifyAuthWatcher();
    }
});
...
lyricsEditBtn.click(() => {
    if (lyricsEditBtn.prop("disabled")) return;
    if (_lyricsEditMode) exitLyricsEditMode();
    else enterLyricsEditMode();
});
lyricsCancelBtn.click(() => exitLyricsEditMode());
lyricsSaveBtn.click(() => saveEditedLyrics());
function syncLyricsAnimationControls() {
    const mode = ((settings.view && settings.view.lyricsAnimation) || lyricsAnimationMode.val() || "leap");
    lyricsAnimationMode.val(mode);
    lyricsAnimationOptions.each(function () {
        const btn = $(this);
        const active = btn.data("lyrics-animation") === mode;
        btn.toggleClass("active", active).attr("aria-pressed", active ? "true" : "false");
    });
}

function setLyricsAnimationMode(mode) {
    mode = mode === "glow" ? "glow" : "leap";
    settings.view.lyricsAnimation = mode;
    syncLyricsAnimationControls();
    saveSettings();
    _lastLyricsRenderKey = "";
    renderLyricsViewer(true);
}

lyricsAnimationOptions.on("click", function () {
    setLyricsAnimationMode($(this).data("lyrics-animation") || "leap");
});
lyricsAnimationMode.on("change", () => setLyricsAnimationMode(lyricsAnimationMode.val() || "leap"));
...
let _initialUpdateCheckStarted = false;
...
function updateTabIsOpen() {
    return $('.nav-btn[data-tab="update"]').hasClass("active") || $("#tab-update").hasClass("active");
}

function renderUpdateCheckResult(data, opts) {
    opts = opts || {};
    const silent = !!opts.silent;
    const showPositiveStatus = !silent || updateTabIsOpen();
    ...
    if (hasUpdate) {
        ...
        _pendingDownloadUrl = data.downloadUrl || null;
        btnDoUpdate.removeClass("hid").prop("disabled", !_pendingDownloadUrl);
        ...
        if (data.releaseNotes && data.releaseNotes.trim()) {
            const notes = escapeHTML(data.releaseNotes.trim()).replace(/\n/g, "<br>");
            notesHtml = "<div class=\"upd-inline-notes\">" + notes + "</div>";
    ...
    btnDoUpdate.addClass("hid").prop("disabled", false);
    updateNavDot.addClass("hid");

    if (showPositiveStatus) {
        ...
        updProgress.addClass("hid").removeClass("active");
        hideUpdateFlowSmooth(0);
    }
...
async function checkForUpdates(opts) {
    ...
    const oldLabel = lbl.text() || "Check for updates";
    ...
    if (!silent) {
        ...
        btnCheckUpdate.addClass("is-checking");
    }
    ...
    _updateCheckInFlight = (async () => {
        try {
            ...
            if (!res.ok || data.error) throw new Error(data.error || "Failed to check for updates");

            return renderUpdateCheckResult(data, { silent });
        } catch(e) {
            if (!silent || updateTabIsOpen()) {
                setUpdateFlow("error", "Check failed. Try again.");
                setUpdStatus("Error: " + e.message, "error", false);
            }
            return false;
        } finally {
            lbl.text(oldLabel === "Checking..." ? "Check for updates" : oldLabel);
...
function scheduleInitialUpdateCheck() {
    if (_initialUpdateCheckStarted) return;
    _initialUpdateCheckStarted = true;
    setTimeout(() => checkForUpdates({ silent: true }), 900);
}
...
let _lastLyricsRenderKey = "";
let _lastLyricsProgressAt = 0;
let _lastLyricsScrolledIndex = -2;
let _lyricsEditTrackId = "";
let _lyricsAnimationFrame = 0;
let _lyricsStableSongKey = "";
let _lyricsStableProgress = 0;
let _lyricsStableActive = { index: -1, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };
let _lyricsSmoothScrollFrame = 0;
let _lyricsSmoothScrollToken = 0;
let _lyricsLastEnteredLine = -2;


function lyricTimeLabel(ms) {
...
function lyricsToEditorText(lines) {
    return (Array.isArray(lines) ? lines : [])
        .map(line => `[${lyricTimeLabel(line.time)}] ${line.text || ""}`)
        .join("\n");
}

function parseEditorLyrics(raw) {
    const rows = String(raw || "").replace(/\r\n/g, "\n").split("\n");
    const parsed = [];
    let lastTime = 0;
    for (const row of rows) {
        if (!row.trim()) continue;
        const match = row.match(/^\s*\[?(\d{1,2}):(\d{2})(?:\.(\d{1,3}))?\]?\s*(.*)$/);
        if (match) {
            const minutes = parseInt(match[1], 10);
            const seconds = parseInt(match[2], 10);
            const fractionRaw = match[3] || "0";
            const msFraction = parseInt(fractionRaw.padEnd(3, "0").slice(0, 3), 10);
            lastTime = (minutes * 60 + seconds) * 1000 + msFraction;
            parsed.push({ time: lastTime, text: match[4] || "" });
        } else {
            parsed.push({ time: lastTime, text: row.trim() });
            lastTime += 2000;
        }
    }
...
function clampWordDuration(value) {
    return Math.max(80, Math.min(12000, Math.round(Number(value) || 0)));
}

function getLyricWordEntries(line, spanMs) {
    const words = splitLyricWords(line && line.text);
    if (!words.length) return [];
    const fallback = clampWordDuration(Math.max(180, (Number(spanMs) || 0) / Math.max(1, words.length)));
    const rawWords = Array.isArray(line && line.words) ? line.words : [];
    return words.map((word, index) => {
        const raw = rawWords[index] || {};
        const duration = clampWordDuration(raw.duration || raw.ms || fallback);
        return { text: word, duration };
    });
}

function getEditorLineSpan(lines, index) {
    const current = lines[index] || {};
    const start = Number(current.time || 0);
    const next = lines[index + 1];
    const duration = Number((window._playback && window._playback.songDuration) || 0);
    const end = next ? Number(next.time || 0) : (duration ? Math.max(start + 900, duration) : start + 2600);
    return Math.max(450, end - start);
}

function formatDurationShort(ms) {
    ms = clampWordDuration(ms);
    return ms >= 1000 ? (ms / 1000).toFixed(ms >= 2000 ? 1 : 2).replace(/\.0$/, "") + "s" : ms + "ms";
}

function collectWordTimingEntries(row) {
    const entries = [];
    row.find(".lyrics-word-time-item").each(function () {
        const item = $(this);
        entries.push({
            text: String(item.data("word") || item.find(".lyrics-word-time-label").text() || ""),
            duration: clampWordDuration(item.find(".lyrics-word-time-input").val())
        });
    });
    return entries;
}

function pickPreservedWordDuration(word, index, previousEntries, fallback) {
    const sameIndex = previousEntries[index];
    if (sameIndex && String(sameIndex.text || "").toLowerCase() === String(word || "").toLowerCase()) {
        return clampWordDuration(sameIndex.duration || fallback);
    }
    const wanted = String(word || "").toLowerCase();
    const foundIndex = previousEntries.findIndex(entry => String(entry.text || "").toLowerCase() === wanted && !entry._used);
    if (foundIndex >= 0) {
        previousEntries[foundIndex]._used = true;
        return clampWordDuration(previousEntries[foundIndex].duration || fallback);
    }
    return clampWordDuration(fallback);
}

function refreshWordTimingSummary(row) {
    const span = Math.max(450, Number(row.data("span")) || 0);
    let total = 0;
    row.find(".lyrics-word-time-input").each(function () {
        total += clampWordDuration($(this).val());
    });
    const diff = Math.round(total - span);
    const summary = row.find(".lyrics-word-timing-summary");
    const ok = Math.abs(diff) <= Math.max(120, span * 0.08);
    summary.toggleClass("is-off", !ok);
    summary.text(`Line ${formatDurationShort(span)} · words ${formatDurationShort(total)}${diff ? ` · ${diff > 0 ? "+" : ""}${diff}ms` : ""}`);
}

function rebuildWordTimingRow(row, options = {}) {
    const previousEntries = options.previousEntries || collectWordTimingEntries(row);
    const text = String(row.find(".lyrics-edit-line-input").val() || "");
    const words = splitLyricWords(text);
    const span = Math.max(450, Number(row.data("span")) || 0);
    const fallback = clampWordDuration(Math.max(180, span / Math.max(1, words.length || 1)));
    const list = row.find(".lyrics-word-timing-list");
    list.empty();

    if (!words.length) {
        list.html('<div class="lyrics-word-timing-empty">Type a lyric line to edit word timing.</div>');
        refreshWordTimingSummary(row);
        return;
    }

    words.forEach((word, index) => {
        const duration = pickPreservedWordDuration(word, index, previousEntries, fallback);
        const item = $(`
            <label class="lyrics-word-time-item" data-word-index="${index}">
              <span class="lyrics-word-time-label"></span>
              <input class="lyrics-word-time-input" type="number" min="80" max="12000" step="10" inputmode="numeric">
              <span class="lyrics-word-time-unit">ms</span>
            </label>
        `);
        item.attr("data-word", word);
        item.find(".lyrics-word-time-label").text(word);
        item.find(".lyrics-word-time-input").val(duration);
        list.append(item);
    });
    refreshWordTimingSummary(row);
}

function getLineSignatureForRender(line) {
    const words = Array.isArray(line && line.words) ? line.words.map(w => `${String(w && w.text || "")}:${Number(w && (w.duration || w.ms)) || 0}`).join(",") : "";
    return `${Number(line && line.time) || 0}:${String(line && line.text || "")}:${words}`;
}

function getLyricsSongKey(pb) {
    pb = pb || window._playback || {};
    return String(pb.songId || `${pb.songName || ""}|${pb.songAuthor || ""}` || "");
}

function getInterpolatedPlaybackProgress() {
    const pb = window._playback || {};
    const base = Number(pb.songProgress || 0);
    const elapsed = pb.isPlaying ? Math.max(0, Date.now() - (_lastLyricsProgressAt || Date.now())) : 0;
    const duration = Number(pb.songDuration || 0);
    let progress = base + elapsed;
    if (duration) progress = Math.min(duration, progress);

    const songKey = getLyricsSongKey(pb);
    const jumpedBack = songKey === _lyricsStableSongKey && progress < _lyricsStableProgress - 900;
    const sameSongSmallBackstep = songKey === _lyricsStableSongKey && progress < _lyricsStableProgress && !jumpedBack;

    if (sameSongSmallBackstep) {
        progress = _lyricsStableProgress;
    } else {
        if (songKey !== _lyricsStableSongKey || jumpedBack) {
            _lyricsStableActive = { index: -1, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };
        }
        _lyricsStableSongKey = songKey;
        _lyricsStableProgress = progress;
    }

    return progress;
}

function getActiveLyricsPosition(lines, progress) {
    if (!Array.isArray(lines) || !lines.length) return { index: -1, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };
    let index = -1;
    for (let i = 0; i < lines.length; i++) {
        if (Number(lines[i].time || 0) <= progress) index = i;
        else break;
    }
    if (index < 0) return { index: -1, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };
    const current = lines[index];
    const next = lines[index + 1];
    const start = Number(current.time || 0);
    const end = next ? Number(next.time || 0) : Math.max(start + 2600, Number(window._playback.songDuration || 0));
    const span = Math.max(450, end - start);
    const ratio = Math.min(0.999, Math.max(0, (progress - start) / span));
    const entries = getLyricWordEntries(current, span);
    const words = entries.map(entry => entry.text);
    if (!words.length) return { index, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };

    let wordIndex = 0;
    let wordRatio = 0;
    const elapsedInLine = Math.max(0, progress - start);
    const hasCustomWordTiming = Array.isArray(current.words) && current.words.length;

    if (hasCustomWordTiming) {
        let cursor = 0;
        for (let i = 0; i < entries.length; i++) {
            const duration = clampWordDuration(entries[i].duration || 0);
            const wordEnd = cursor + duration;
            if (elapsedInLine < wordEnd || i === entries.length - 1) {
                wordIndex = i;
                wordRatio = Math.min(0.999, Math.max(0, (elapsedInLine - cursor) / Math.max(80, duration)));
                if (elapsedInLine >= wordEnd && i === entries.length - 1) wordRatio = 0.999;
                break;
            }
            cursor = wordEnd;
        }
    } else {
        const exactWord = ratio * words.length;
        wordIndex = Math.min(words.length - 1, Math.floor(exactWord));
        wordRatio = Math.min(0.999, Math.max(0, exactWord - wordIndex));
    }

    const currentWordLength = Array.from(words[wordIndex] || "").length;
    const revealChars = currentWordLength ? Math.max(1, Math.min(currentWordLength, Math.ceil(wordRatio * currentWordLength))) : 0;
    return { index, wordIndex, ratio, wordRatio, revealChars };
}

function getStableActiveLyricsPosition(lines, progress, pb) {
    const active = getActiveLyricsPosition(lines, progress);
    const songKey = getLyricsSongKey(pb || window._playback || {});

    if (songKey !== _lyricsStableSongKey || active.index < 0 || active.wordIndex < 0) {
        _lyricsStableSongKey = songKey;
        _lyricsStableActive = active;
        return active;
    }

    const previous = _lyricsStableActive || { index: -1, wordIndex: -1 };
    const movedBack = active.index < previous.index || (active.index === previous.index && active.wordIndex < previous.wordIndex);
    const explicitSeekBack = progress < _lyricsStableProgress - 900;

    if (movedBack && !explicitSeekBack) {
        return previous;
    }
...
function renderActiveWord(word, options = {}) {
    const chars = Array.from(String(word || ""));
    const total = Math.max(1, chars.length);
    const mode = options.mode || "leap";
    const wordRatio = Math.max(0, Math.min(0.999, Number(options.wordRatio) || 0));

    return chars.map((ch, i) => {
        const classes = ["lyric-char"];
        if (mode === "leap") classes.push("leap-char");
        if (mode === "glow") classes.push("glow-char");
        const visualStyle = mode === "leap"
            ? getLeapCharInlineStyle(i, total, wordRatio)
            : `--char-index:${i};--char-total:${total};--char-delay:${Math.min(10, i) * 28}ms`;
        return `<span class="${classes.join(" ")}" data-char-index="${i}" style="${visualStyle}">${escapeHTML(ch)}</span>`;
    }).join("");
}
...
function getLeapCharVisual(i, total, wordRatio) {
    total = Math.max(1, Number(total) || 1);
    wordRatio = Math.max(0, Math.min(0.999, Number(wordRatio) || 0));

    const revealHead = Math.min(total + 0.9, Math.max(0.32, wordRatio * total + 0.62));
    const phase = revealHead - i;
    ...
    if (phase >= 1) {
        ...
        const eSoft = easeOutCubic(e);
        opacity = 0.28 + 0.72 * e;
        x = -22 * (1 - e);
        y = -1.0 * Math.sin(e * Math.PI);
        skew = -6 * (1 - eSoft);
        scale = 0.948 + 0.052 * e;
        glow = 0.20 + 0.44 * e;
    } else if (phase >= -1.2) {
        const e = easeOutCubic(1 + phase / 1.2);
        opacity = 0.04 + 0.22 * e;
        x = -36 + 14 * e;
        y = 0.3;
        skew = -9 + 3 * e;
    ...
    return { opacity, x, y, skew, scale, glow };
}

function getLeapCharInlineStyle(i, total, wordRatio) {
    const v = getLeapCharVisual(i, total, wordRatio);
    const shadow = v.glow > 0.01
        ? `0 0 ${(3 + v.glow * 6).toFixed(1)}px rgba(255,255,255,${(0.14 + v.glow * 0.30).toFixed(3)}), 0 0 ${(10 + v.glow * 18).toFixed(1)}px var(--accent), 0 0 ${(20 + v.glow * 28).toFixed(1)}px color-mix(in srgb, var(--accent) 60%, transparent)`
        : "none";
    return [
        `--char-index:${i}`,
        `--char-total:${total}`,
        `opacity:${v.opacity.toFixed(3)}`,
        `transform:translate3d(${v.x.toFixed(2)}px, ${v.y.toFixed(2)}px, 0) skewX(${v.skew.toFixed(2)}deg) scale(${v.scale.toFixed(3)})`,
        `filter:none`,
        `text-shadow:${shadow}`
    ].join(";");
}

function updateLeapLetters() {
    if (_lyricsEditMode) return;
    const animationMode = (settings.view && settings.view.lyricsAnimation) || "leap";
    if (animationMode !== "leap") return;

    const pb = window._playback || {};
    const lines = Array.isArray(pb.lines) ? pb.lines : [];
    if (!lines.length) return;

    const progress = getInterpolatedPlaybackProgress() + 120;
    const active = getStableActiveLyricsPosition(lines, progress, pb);
    if (active.index < 0 || active.wordIndex < 0) return;

    const wordEl = lyricsViewer.find(".leap-current-word")[0];
    if (!wordEl) return;
    const chars = Array.from(wordEl.querySelectorAll(".leap-char"));
    if (!chars.length) return;

    const total = chars.length;

    chars.forEach((char, i) => {
        const v = getLeapCharVisual(i, total, active.wordRatio);
        char.style.opacity = v.opacity.toFixed(3);
        char.style.transform = `translate3d(${v.x.toFixed(2)}px, ${v.y.toFixed(2)}px, 0) skewX(${v.skew.toFixed(2)}deg) scale(${v.scale.toFixed(3)})`;
        char.style.filter = "none";
        char.style.textShadow = v.glow > 0.01
            ? `0 0 ${(3 + v.glow * 6).toFixed(1)}px rgba(255,255,255,${(0.14 + v.glow * 0.30).toFixed(3)}), 0 0 ${(10 + v.glow * 18).toFixed(1)}px var(--accent), 0 0 ${(20 + v.glow * 28).toFixed(1)}px color-mix(in srgb, var(--accent) 60%, transparent)`
            : "none";
    });
}
...
function lyricsAnimationLoop() {
    updateLeapLetters();
    _lyricsAnimationFrame = requestAnimationFrame(lyricsAnimationLoop);
}
...
function smoothScrollLyricsToActive(activeEl, activeIndex) {
    const container = lyricsViewer && lyricsViewer[0];
    if (!container || !activeEl || !isLyricsTabActive()) return;

    const containerHeight = container.clientHeight || 0;
    const maxScroll = Math.max(0, container.scrollHeight - containerHeight);
    const target = Math.max(0, Math.min(maxScroll, activeEl.offsetTop - (containerHeight * 0.48) + (activeEl.offsetHeight * 0.50)));
    const start = container.scrollTop;
    const distance = target - start;
    if (Math.abs(distance) < 2) return;

    if (_lyricsSmoothScrollFrame) cancelAnimationFrame(_lyricsSmoothScrollFrame);
    const token = ++_lyricsSmoothScrollToken;
    const distanceAbs = Math.abs(distance);
    const duration = Math.max(780, Math.min(1450, 720 + distanceAbs * 1.45));
    const startedAt = performance.now();

    const step = (now) => {
        if (token !== _lyricsSmoothScrollToken) return;
        const t = Math.min(1, (now - startedAt) / duration);
        container.scrollTop = start + distance * easeInOutSmoother(t);
        if (t < 1) {
            _lyricsSmoothScrollFrame = requestAnimationFrame(step);
        } else {
            container.scrollTop = target;
            _lyricsSmoothScrollFrame = 0;
        }
    };
    _lyricsSmoothScrollFrame = requestAnimationFrame(step);
}
...
function getActiveTabKey() {
    const fromSafe = (typeof _safeActiveTab !== "undefined" && _safeActiveTab) ? _safeActiveTab : "";
    const visible = ($(".tab.active").first().attr("id") || "").replace("tab-", "");
    return visible || fromSafe || "auth";
}

function isLyricsTabActive() {
    return getActiveTabKey() === "lyrics" && $("#tab-lyrics").hasClass("active");
}

function renderLyricsViewer(force) {
    const pb = window._playback || {};
    const lines = Array.isArray(pb.lines) ? pb.lines : [];
    const source = pb.lyricsSource ? ` · ${pb.lyricsSource}` : "";
    if (pb.songName) {
        lyricsSongMeta.text(`${pb.songName}${pb.songAuthor ? " — " + pb.songAuthor : ""}${source}`);
    } else {
        lyricsSongMeta.text("Play a song to preview lyrics here.");
    }

    if (_lyricsEditMode) return;
    if (!lines.length) {
        _lastLyricsRenderKey = "";
        _lastLyricsScrolledIndex = -2;
        _lyricsLastEnteredLine = -2;
        if (_lyricsSmoothScrollFrame) cancelAnimationFrame(_lyricsSmoothScrollFrame);
        _lyricsSmoothScrollFrame = 0;
        _lyricsStableActive = { index: -1, wordIndex: -1, ratio: 0, wordRatio: 0, revealChars: 0 };
        lyricsViewer.html('<div class="lyrics-empty">Waiting for synced lyrics…</div>');
        lyricsEditBtn.prop("disabled", true).addClass("disabled");
        return;
    }
    lyricsEditBtn.prop("disabled", false).removeClass("disabled");

    const animationMode = (settings.view && settings.view.lyricsAnimation) || "leap";
    const progress = getInterpolatedPlaybackProgress() + (animationMode === "leap" ? 150 : 0);
    const active = getStableActiveLyricsPosition(lines, progress, pb);
    const renderKey = `${animationMode}|${pb.songId || pb.songName}|${lines.length}|${active.index}|${active.wordIndex}|${lines.map(getLineSignatureForRender).join('|')}`;
    if (!force && renderKey === _lastLyricsRenderKey) return;
    _lastLyricsRenderKey = renderKey;

    const leapMode = animationMode === "leap";
    lyricsViewer.toggleClass("lyrics-leap-mode", leapMode);
    lyricsViewer.toggleClass("lyrics-glow-mode", !leapMode);

    const html = lines.map((line, index) => {
        if (leapMode && index > active.index) return "";
        const text = String(line.text || "").trim() || "♪";
        const words = text.split(/(\s+)/);
        let wordCounter = -1;
        let visibleWordSeen = false;
        const wordHtml = words.map(part => {
            if (/^\s+$/.test(part)) return visibleWordSeen ? escapeHTML(part) : "";
            wordCounter++;
            const isActiveLine = index === active.index;
            const isCurrentWord = isActiveLine && wordCounter === active.wordIndex;
            const isPastWord = index < active.index || (isActiveLine && wordCounter < active.wordIndex);
            const isFutureWord = isActiveLine && wordCounter > active.wordIndex;

            if (leapMode && isFutureWord) return "";
            visibleWordSeen = true;

            const cls = ["lyrics-word"];
            if (isCurrentWord) cls.push("active-word", leapMode ? "leap-current-word" : "glow-current-word");
            else if (isPastWord && wordCounter === active.wordIndex - 1 && isActiveLine) cls.push("just-passed-word", "sung-word");
            else if (isPastWord) cls.push("sung-word");

            const content = isCurrentWord
                ? renderActiveWord(part, { mode: leapMode ? "leap" : "glow", wordRatio: active.wordRatio })
                : escapeHTML(part);
            const style = isCurrentWord
                ? ` style="--word-index:${wordCounter};--word-length:${Array.from(part).length};"`
                : "";
            const wordAttr = isCurrentWord ? ` data-word="${escapeHTML(part)}"` : "";
            return `<span class="${cls.join(" ")}" data-word-index="${wordCounter}"${wordAttr}${style}>${content}</span>`;
        }).join("");
        const entering = index === active.index && _lyricsLastEnteredLine !== active.index;
        const cls = index === active.index ? `lyric-line active${entering ? " entering-line" : ""}` : (index < active.index ? "lyric-line passed" : "lyric-line");
        return `<div class="${cls}" data-line-index="${index}"><span class="lyric-time">${lyricTimeLabel(line.time)}</span><span class="lyric-text">${wordHtml || '<span class="lyrics-word active-word leap-current-word">♪</span>'}</span></div>`;
    }).join("");
    lyricsViewer.html(html || '<div class="lyrics-empty">Waiting for the first lyric…</div>');
    updateLeapLetters();

    const activeEl = lyricsViewer.find('.lyric-line.active')[0];
    if (activeEl && isLyricsTabActive() && _lastLyricsScrolledIndex !== active.index) {
        _lastLyricsScrolledIndex = active.index;
        _lyricsLastEnteredLine = active.index;
        smoothScrollLyricsToActive(activeEl, active.index);
    }
}

function buildLyricsEditor(lines) {
    const safeLines = Array.isArray(lines) ? lines : [];
    lyricsEditorList.empty();
    safeLines.forEach((line, index) => {
        const span = getEditorLineSpan(safeLines, index);
        const entries = getLyricWordEntries(line, span);
        const row = $(`
            <div class="lyrics-edit-row" data-time="${Number(line.time) || 0}" data-index="${index}" data-span="${span}">
              <div class="lyrics-edit-time">${lyricTimeLabel(line.time)}</div>
              <div class="lyrics-edit-main">
                <textarea class="lyrics-edit-line-input" rows="1" spellcheck="false"></textarea>
                <div class="lyrics-word-timing">
                  <div class="lyrics-word-timing-head">
                    <span>Word timing</span>
                    <span class="lyrics-word-timing-summary"></span>
                  </div>
                  <div class="lyrics-word-timing-list"></div>
                </div>
              </div>
            </div>
        `);
        const input = row.find(".lyrics-edit-line-input");
        input.val(String(line.text || ""));
        autoSizeLyricsInput(input[0]);
        lyricsEditorList.append(row);
        rebuildWordTimingRow(row, { previousEntries: entries });
    });
    lyricsEditorText.val(lyricsToEditorText(safeLines));
}

function autoSizeLyricsInput(el) {
    if (!el) return;
    el.style.height = "auto";
    el.style.height = Math.max(44, el.scrollHeight) + "px";
}

$(document).on("input", ".lyrics-edit-line-input", function () {
    autoSizeLyricsInput(this);
    rebuildWordTimingRow($(this).closest(".lyrics-edit-row"));
});

$(document).on("input change", ".lyrics-word-time-input", function () {
    const input = $(this);
    input.val(clampWordDuration(input.val()));
    refreshWordTimingSummary(input.closest(".lyrics-edit-row"));
});

function getLyricsFromEditor() {
    const rows = lyricsEditorList.find(".lyrics-edit-row");
    if (rows.length) {
        const parsed = [];
        rows.each(function () {
            const row = $(this);
            const text = String(row.find(".lyrics-edit-line-input").val() || "");
            const textWords = splitLyricWords(text);
            const controls = row.find(".lyrics-word-time-item");
            const words = textWords.map((word, index) => {
                const control = controls.eq(index);
                const duration = clampWordDuration(control.find(".lyrics-word-time-input").val());
                return { text: word, duration };
            });
            const line = {
                time: Math.max(0, Number(row.data("time")) || 0),
                text
            };
    ...
    return parseEditorLyrics(lyricsEditorText.val());
}
...
function enterLyricsEditMode() {
    ...
    _lyricsEditTrackId = pb.songId || "";
    buildLyricsEditor(lines);
    lyricsEditor.removeClass("hid");
    lyricsViewer.addClass("hid");
    lyricsEditBtn.addClass("active");
    const firstInput = lyricsEditorList.find(".lyrics-edit-line-input").first()[0];
...
function exitLyricsEditMode() {
    ...
    _lyricsEditTrackId = "";
    lyricsEditor.addClass("hid");
    lyricsViewer.removeClass("hid");
    lyricsEditBtn.removeClass("active");
    renderLyricsViewer(true);
}
...
async function saveEditedLyrics() {
    let lines;
    try {
        lines = getLyricsFromEditor();
    } catch (e) {
        setUpdStatus(e.message, "error", false);
        return;
    }
    if (!lines.length) {
        setUpdStatus("Add at least one lyric line before saving.", "error", false);
        return;
    }
    const pb = window._playback || {};
    const label = lyricsSaveBtn.find(".btn-label");
    const old = label.text();
    lyricsSaveBtn.addClass("is-loading").prop("disabled", true);
    lyricsCancelBtn.prop("disabled", true);
    label.text("Saving...");
    try {
        const res = await fetch("/api/lyrics/custom", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ trackId: _lyricsEditTrackId || pb.songId || "", songName: pb.songName || "", songAuthor: pb.songAuthor || "", lines })
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Could not save lyrics");
        window._playback = { ...window._playback, ...data, type: "playback", lines: Array.isArray(data.lines) ? data.lines : lines, hasLyrics: true };
        _lastLyricsProgressAt = Date.now();
        _lastLyricsRenderKey = "";
        exitLyricsEditMode();
        setUpdStatus("Lyrics saved for this song.", "success", false);
    } catch (e) {
        setUpdStatus("Lyrics save failed: " + e.message, "error", false);
    } finally {
        lyricsSaveBtn.removeClass("is-loading").prop("disabled", false);
        lyricsCancelBtn.prop("disabled", false);
        label.text(old || "Save all lyrics");
    }
}

setInterval(() => renderLyricsViewer(false), 80);
if (!_lyricsAnimationFrame) _lyricsAnimationFrame = requestAnimationFrame(lyricsAnimationLoop);
...
let _spotifyAuthPollStopTimer = null;

function startSpotifyAuthWatcher() {
    clearInterval(_spotifyAuthPollTimer);
    clearTimeout(_spotifyAuthPollStopTimer);

    spotifyAuthorizedIndicator.addClass("hid");
    authorizeButton.addClass("is-loading");
    const label = authorizeButton.find(".btn-label");
    const originalLabel = label.text();
    label.text("Waiting for Spotify...");

    let lastHadSpotify = !!(settings.credentials && (settings.credentials.refreshToken || settings.credentials.code));

    const finish = () => {
        clearInterval(_spotifyAuthPollTimer);
        clearTimeout(_spotifyAuthPollStopTimer);
        _spotifyAuthPollTimer = null;
        _spotifyAuthPollStopTimer = null;
        authorizeButton.removeClass("is-loading");
        label.text(originalLabel || "Authorize Spotify");
        updateConnectionStatus();
    };

    const tick = async () => {
        await refreshSettingsFromServer();
        const hasSpotify = !!(settings.credentials && (settings.credentials.refreshToken || settings.credentials.code));
        if (hasSpotify && !lastHadSpotify) {
            spotifyAuthorizedIndicator.removeClass("hid");
            finish();
        }
        lastHadSpotify = hasSpotify;
    };

    _spotifyAuthPollTimer = setInterval(tick, 1000);
    _spotifyAuthPollStopTimer = setTimeout(finish, 45000);
    setTimeout(tick, 700);
}

window.addEventListener("message", (event) => {
    if (event && event.data && event.data.type === "spotify_authorized") {
        refreshSettingsFromServer().then(() => updateConnectionStatus());
    }
});
...
function loadSettings(raw) {
    ...
    try {
        ...
        settings.view.lyricsAnimation = settings.view.lyricsAnimation || "leap";
        enableTimestampCheckbox.prop("checked", settings.view.timestamp);
        enableLabelCheckbox.prop("checked", settings.view.label);
        lyricsAnimationMode.val(settings.view.lyricsAnimation);
        syncLyricsAnimationControls();
        ...
        if (authorized && _spotifyAuthPollTimer) {
            clearInterval(_spotifyAuthPollTimer);
            clearTimeout(_spotifyAuthPollStopTimer);
            _spotifyAuthPollTimer = null;
            _spotifyAuthPollStopTimer = null;
            authorizeButton.removeClass("is-loading");
            authorizeButton.find(".btn-label").text("Authorize Spotify");
        }
...
window._playback = { isPlaying: false, songId: "", songName: "", songAuthor: "", songDuration: 0, songProgress: 0, hasLyrics: false, lyricsActive: false, lyricsSource: "", activeLineIndex: -1, lines: [] };
    ...
    ws.onmessage = (msg) => {
        try {
            ...
            if (data.type === "playback") {
                ...
                _lastLyricsProgressAt = Date.now();
                updateConnectionStatus();
                renderLyricsViewer(false);
            } else if (data.type === "lyrics") {
                window._playback = { ...window._playback, ...data };
                _lastLyricsProgressAt = Date.now();
                renderLyricsViewer(true);
                updateConnectionStatus();
                ...
                setUpdateFlow("done", data.message || "Update complete. Restarting automatically...");
...
(function rapidSmoothTabsV703() {
    const ORDER = ["auth", "display", "lyrics", "timing", "theme", "update"];
