...
function renderActiveWord(word, options = {}) {
    ...
    return chars.map((ch, i) => {
        ...
        const visualStyle = mode === "leap"
            ...
            : `--char-index:${i};--char-total:${total};--char-delay:${Math.min(7, i) * 24}ms`;
...
function getLeapCharVisual(i, total, wordRatio) {
    ...
    if (phase >= 1) {
        ...
        opacity = 0.36 + 0.64 * e;
        x = -24 * (1 - e);
        y = -1.2 * Math.sin(e * Math.PI);
        skew = -7 * (1 - e);
        scale = 0.945 + 0.055 * e;
        glow = 0.24 + 0.40 * e;
    } else if (phase >= -1.05) {
        const e = easeOutCubic(1 + phase / 1.05);
        opacity = 0.06 + 0.24 * e;
        x = -38 + 14 * e;
        y = 0.4;
        skew = -10 + 3 * e;
...
function getLeapCharInlineStyle(i, total, wordRatio) {
    ...
    const shadow = v.glow > 0.01
        ? `0 0 ${(4 + v.glow * 7).toFixed(1)}px rgba(255,255,255,${(0.18 + v.glow * 0.32).toFixed(3)}), 0 0 ${(11 + v.glow * 17).toFixed(1)}px var(--accent)`
        : "none";
    return [
        `--char-index:${i}`,
        `--char-total:${total}`,
